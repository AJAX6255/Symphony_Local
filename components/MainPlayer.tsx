...
// Desired max frequency for bird songs (if the file supports it)
...
const generateBirdSongColormap = () => {
  ...
  for (let i = 0; i < 256; i++) {
    ...
    const rangeRatio = range === 0 ? 0 : (ratio - start.pos) / range;
    const r = Math.round(start.color[0] + (end.color[0] - start.color[0]) * rangeRatio);
    const g = Math.round(start.color[1] + (end.color[1] - start.color[1]) * rangeRatio);
    const b = Math.round(start.color[2] + (end.color[2] - start.color[2]) * rangeRatio);
    colormap.push([r, g, b]);
  }
  return colormap;
};
...
const MainPlayer: React.FC<MainPlayerProps> = ({ track, micStream, onNext, onPrev }) => {
  ...
  // State to track the ACTUAL max frequency being displayed (for the labels)
  const [activeFreqMax, setActiveFreqMax] = useState(TARGET_FREQ_MAX);

  // --- ADAPTIVE CANVAS OBSERVER ---
  // Watches the spectrogram container and forces the canvas to fit 100% height
  useLayoutEffect(() => {
    ...
    const observer = new MutationObserver((mutations) => {
      mutations.forEach(() => {
        ...
        const wrapper = spectrogramRef.current?.querySelector('div');
    ...
    observer.observe(spectrogramRef.current, { childList: true, subtree: true, attributes: true });
  ...
  const initWaveSurfer = useCallback(() => {
    ...
    // 1. Initialize WaveSurfer WITHOUT the spectrogram first
    const ws = WaveSurfer.create({
      ...
      height: 140,
      interact: true,
      plugins: [], // Defer plugin initialization
    });

    // 2. Initialize Spectrogram ONLY after we know the file's properties
    ws.on('decode', () => {
      const decodedData = ws.getDecodedData();
      if (decodedData) {
        // Calculate the file's actual physical limit (Nyquist)
        const fileNyquist = decodedData.sampleRate / 2;
        
        // If the file's quality is lower than our target (12k), cap the display at the file limit
        // so we don't get white space. Otherwise, crop to 12k.
        const dynamicMax = Math.min(TARGET_FREQ_MAX, fileNyquist);
        
        setActiveFreqMax(dynamicMax);
        ...
        ws.registerPlugin(
          Spectrogram.create({
            ...
            splitChannels: false,
            frequencyMin: 0,
            frequencyMax: dynamicMax, // Use the calculated limit
  ...
  const runLiveCalibration = async () => {
    // ... (Your existing calibration logic stays the same)
    ...
    let stepCount = 0;
    for (let freq = 10; freq <= TARGET_FREQ_MAX; freq += 200) {
    ...
    const interval = setInterval(() => {
      ...
      if (currentFreq > TARGET_FREQ_MAX) clearInterval(interval);
  ...
  useEffect(() => {
    // ... (Your existing live mic logic stays the same, using TARGET_FREQ_MAX)
    ...
    const renderLive = () => {
      ...
      const maxFreqIndex = Math.floor((TARGET_FREQ_MAX / nyquist) * bufferLength);
  ...
  const takeSnapshot = () => {
    const targetCanvas = micStream
      ? liveCanvasRef.current
      : (spectrogramRef.current?.querySelector('canvas') as HTMLCanvasElement | null);
  ...
  // Helper to generate dynamic labels based on what the spectrogram is actually showing
  const getFreqLabels = (maxFreq: number) => {
    return [
      { label: `${(maxFreq / 1000).toFixed(1)}k`, top: '2%' },
      { label: `${(maxFreq * 0.83 / 1000).toFixed(1)}k`, top: '16.6%' },
      { label: `${(maxFreq * 0.66 / 1000).toFixed(1)}k`, top: '33.3%' },
      { label: `${(maxFreq * 0.5 / 1000).toFixed(1)}k`, top: '50%' },
      { label: `${(maxFreq * 0.33 / 1000).toFixed(1)}k`, top: '66.6%' },
      { label: `${(maxFreq * 0.16 / 1000).toFixed(1)}k`, top: '83.3%' },
      { label: '0 Hz', top: '96%' },
    ];
  };

  const currentLabels = getFreqLabels(micStream ? TARGET_FREQ_MAX : activeFreqMax);
  ...
  return (
    <div className="flex-1 flex flex-col bg-gray-950 overflow-y-auto p-6 md:p-10 custom-scrollbar">
      <style>{`
        ...
        .wavesurfer-spectrogram > div:not(canvas) {
      ...
      {!track && !micStream ? (
        ...
        <div className="max-w-6xl mx-auto w-full space-y-8">
          <div className="flex items-center justify-between">
            <div className="min-w-0 flex-1">
              <h1 className="text-3xl md:text-4xl font-extrabold text-white mb-2 truncate">
                {micStream ? 'Live Field Analysis' : track?.name}
              </h1>
            ...
            <div className="flex items-center space-x-3">
              {micStream && (
                <button
                  ...
                  className={`flex items-center space-x-2 px-4 py-2 border rounded-xl transition-all uppercase tracking-widest text-[10px] font-black shadow-2xl shadow-black ${isCalibrating ? 'bg-blue-600 border-blue-400 text-white animate-pulse' : 'bg-gray-900 hover:bg-gray-800 border-gray-700 text-gray-400 hover:text-blue-400 hover:border-blue-900'}`}
                >
          ...
          <div className="bg-gray-900 rounded-3xl p-6 md:p-8 shadow-3xl border border-gray-800 space-y-8">
            {!micStream && <div ref={waveformRef} className="w-full" />}
            <div className={`pt-6 ${!micStream ? 'border-t border-gray-800' : ''}`}>
              <div className="flex items-center justify-between mb-6">
                <div className="space-y-1">
                  <h3 className="text-xs uppercase tracking-[0.2em] text-gray-500 font-black">
                    {micStream ? 'Waterfall Spectrogram' : 'Static Spectrogram'}
                  </h3>
                  <p className="text-[10px] text-gray-700 font-mono italic">
                    {micStream ? 'Real-time analysis @ 48kHz' : `Visualized range: 0 — ${(activeFreqMax / 1000).toFixed(1)} kHz`}
                  </p>
                </div>
              </div>
              <div className="spectrogram-parent-container w-full rounded-3xl border-4 border-gray-800/50 shadow-2xl overflow-hidden bg-black">
                <div className="freq-gutter">
                  {currentLabels.map((f, i) => (
                    <div
                      key={i}
                      className="absolute left-0 w-full flex items-center pr-2"
                      style={{ top: f.top, transform: 'translateY(-50%)' }}
                    >
                      <span className="flex-1 text-[10px] font-black text-gray-200 font-mono tracking-tighter text-right">
                        {f.label}
                      </span>
                      <div className="w-2 h-[2px] bg-gray-400 ml-2" />
                    </div>
                  ))}
                </div>
                <div className="analysis-area">
                  {micStream ? (
                    <canvas id="live-waterfall" ref={liveCanvasRef} width={1200} height={256} className="w-full h-[256px]" />
                  ) : (
                    <div ref={spectrogramRef} className="w-full bg-black h-[256px]" />
                  )}
                  <div className="absolute inset-0 pointer-events-none opacity-[0.05] z-30">
                    {[1, 2, 3, 4, 5].map((i) => (
              ...
              <div className="mt-4 flex items-center justify-between text-[10px] font-mono pl-20">
                <div className="text-gray-300">
                  {isBuffering
                    ? 'DECODING MP3 DATA...'
                    : isCalibrating
                    ? `SCALE VERIFICATION: ${calibFreq} Hz (Audible Signal)`
                    : error
                    ? `ERROR: ${error}`
                    : 'SYSTEM READY'}
                </div>
                <div className="text-gray-500">
                  {micStream ? `GAIN: ${micGain.toFixed(1)}x` : `VOL: ${Math.round(volume * 100)}%`}
                </div>
              </div>
            </div>
            <div className="flex flex-col md:flex-row items-center justify-between gap-8 pt-8 border-t border-gray-800">
              {!micStream ? (
                <div className="flex items-center space-x-6">
                  <button onClick={onPrev} className="text-gray-500 hover:text-white transition-colors" title="Previous Track"><svg className="w-8 h-8" fill="currentColor" viewBox="0 0 24 24"><path d="M6 6h2v12H6zm3.5 6l8.5 6V6z" /></svg></button>
                  <button onClick={() => wavesurfer.current?.playPause()} className="p-5 bg-blue-600 hover:bg-blue-500 rounded-full transition-all transform hover:scale-105 shadow-lg shadow-blue-600/20" title={isPlaying ? 'Pause' : 'Play'}>
                  ...
                  <button onClick={() => { wavesurfer.current?.stop(); setIsPlaying(false); }} className="p-5 bg-gray-800 hover:bg-gray-700 rounded-full transition-all transform hover:scale-105 shadow-lg border border-gray-700" title="Stop"><svg className="w-8 h-8 text-white" fill="currentColor" viewBox="0 0 24 24"><path d="M6 6h12v12H6z" /></svg></button>
                  <button onClick={onNext} className="text-gray-500 hover:text-white transition-colors" title="Next Track"><svg className="w-8 h-8" fill="currentColor" viewBox="0 0 24 24"><path d="M6 18l8.5-6L6 6v12zM16 6v12h2V6h-2z" /></svg></button>
                </div>
              ) : (
